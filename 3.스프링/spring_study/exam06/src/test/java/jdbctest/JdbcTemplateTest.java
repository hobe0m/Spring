...
@ContextConfiguration(classes = AppCtx.class) // 설정 클래스로 쓸 클래스들 입력
@Transactional
public class JdbcTemplateTest {
        ...
        @Transactional // 프록시가 구현된 형태
        void insertTest() {
            
            // DataAccessException의 하위객체이고, RuntimeException이므로 예외 처리를 하지 않고 실행해도 된다.
            
            String sql = " INSERT INTO MEMBER (USER_NO, USER_ID, USER_PW, USER_NM, EMAIL) " +
                    " VALUES (SEQ_MEMBER.nextval, ?, ?, ?, ?)"; 

            int affectedRows = jdbcTemplate.update(sql, "USER102", "123456", "사용자102", "user102@test.org");
